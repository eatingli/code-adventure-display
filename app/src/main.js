...
let view = new View(WIDTH, HEIGHT, 26, 18, 2 * 5 * Math.PI / 360, [145, 10, 10, 10]);
...
function render() {
    ...
    function drawM(x, y, mClass) {
        ...
        let vOffset = rect.y + view.grid.h * mConfig.lifeOffset;
    ...
    // Building
    function drawB(x, y, bClass) {
        let alpha = 0.9;
        let config = BuildingConfigs.get(bClass);
        let img = imgs.get(config.img);
        drawBuilding(ctx, config.scale, x, y, 3.2, img, config.h, config.v, alpha);
    }
